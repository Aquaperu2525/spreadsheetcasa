...
import { BaseRepository, FullCollectionMetadata, MetadataStorageConfig } from 'src/metadata/MetadataStorage';
import { getMetadataStorage } from 'src/metadata/MetadataUtils';
import { ICustomQuery, IEntity, IEntityConstructor, IFireOrmQueryLine, IFirestoreVal, IOrderByParams, IQueryBuilder, IRepository, ITransactionReferenceStorage, IWherePropParam, PartialBy } from 'src/types/types';
import { NoMetadataError, ValidationError } from './error';
import { isDocumentReference, isGeoPoint, isObject, isTimestamp } from './guards';
  ...
  implements IRepository<T>
{
  ...
  protected readonly firestoreColRef: any//CollectionReference;

  constructor(pathOrConstructor: string | IEntityConstructor) {
    super();

    const { getCollection, config, 
        //firestoreRef 
    } = getMetadataStorage();
...
function serializeEntity<T extends IEntity>(obj: T, subCollections: import("src/metadata/MetadataStorage").SubCollectionMetadataWithSegments[]): Record<string, unknown> {
