import { FirestoreOperators, ICustomQuery, IEntity, IFireOrmQueryLine, IFirestoreVal, IOrderByParams, IQueryBuilder, IQueryExecutor, IWherePropParam, OrderByDirection } from "src/types/types";
...
export class QueryBuilder<T extends IEntity> implements IQueryBuilder<T> {
    ...
    orderByAscending(prop: IWherePropParam<T>) {
      ...
      this.orderByObj = {
        ...
        directionStr:OrderByDirection.ACENDENTE ,
      };
    ...
    orderByDescending(prop: IWherePropParam<T>) {
      ...
      this.orderByObj = {
        fieldPath: this.extractWhereParam(prop),
        directionStr: OrderByDirection.DESCENDENTE,
      };
